...
if isempty(h)
    error(message('comm:syndtable:EmptyH'));
end

[n_k,n]=size(h);                    % n_k denotes n-k
if n_k >= n || ~isequal(size(size(h)),[1 2])
    error(message('comm:syndtable:LessRowsMoreColumns'));
end

if ~isnumeric(h) || max(max(h < 0)) || max(max(h)) > 1 || (max(max(floor(h) ~= h)))
    error(message('comm:syndtable:NonBinaryNosInH'));
end
...
[a,b,c] = unique(synSingleErr,'legacy');
table(synSingleErr(b)+1,:) = E(b,:);

% Update record of empty rows
emptyRows = setdiff(emptyRows,synSingleErr(b)+1,'legacy');
